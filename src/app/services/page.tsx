...
function ServicesContent() {
  ...
  return (
    <div className="pt-28 sm:pt-32 pb-20 sm:pb-24 min-h-screen bg-[#F8FAFC] dark:bg-[#0B1320] text-slate-900 dark:text-white relative overflow-hidden transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        
        {/* Page Header */}
        <div className="text-center max-w-3xl mx-auto space-y-3 sm:space-y-4 mb-12 sm:mb-16">
          <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-brand-steel/10 text-brand-navy dark:text-brand-steel-light text-[11px] sm:text-xs font-bold uppercase tracking-wider">
            Dual-Category Capabilities
          </div>
          <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold tracking-tight text-slate-900 dark:text-white leading-tight break-words">
        ...
        <div className="bg-white dark:bg-[#111C2E] rounded-2xl sm:rounded-3xl p-3.5 sm:p-6 shadow-sm border border-slate-200 dark:border-slate-800 mb-10 sm:mb-12 flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3 sm:gap-4">
          ...
          <div className="relative w-full md:w-80">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2.5 rounded-xl sm:rounded-full bg-slate-50 dark:bg-slate-800/90 border border-slate-300 dark:border-slate-700 text-xs text-slate-900 dark:text-white placeholder:text-slate-400 focus:outline-none focus:border-brand-steel transition-all"
            />
          </div>
        ...
        {filteredServices.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 mb-20">
            {filteredServices.map((service, idx) => {
              ...
              return (
                <div
                  key={service.id}
                  className={`bg-white dark:bg-[#111C2E] rounded-3xl p-6 sm:p-8 shadow-sm border flex flex-col justify-between transition-all duration-200 ${
                    isBusiness
                      ? "border-slate-200 dark:border-slate-800 hover:border-brand-steel/50"
                      : "border-slate-200 dark:border-slate-800 hover:border-brand-rust/50"
                  }`}
                >
                  <div className="space-y-4">
                    ...
                    {/* Benchmark KPI */}
                    <div className="p-3 rounded-2xl bg-slate-50 dark:bg-[#0E1728] border border-slate-200/80 dark:border-slate-800">
                    ...
                    <div className="space-y-1.5 pt-1">
                      {service.deliverables.slice(0, 2).map((del, dIdx) => (
                        <div key={dIdx} className="flex items-start gap-2 text-xs text-slate-600 dark:text-slate-300 font-normal">
                          <CheckCircle2 className={`w-3.5 h-3.5 mt-0.5 shrink-0 ${isBusiness ? "text-brand-navy dark:text-brand-steel" : "text-brand-rust"}`} />
                  ...
                  <div className="pt-6 mt-6 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                    <button
                      ...
                      className="text-xs font-bold text-slate-700 dark:text-slate-300 hover:text-brand-rust transition-colors"
                    >
          ...
          <div className="text-center py-20 bg-white dark:bg-[#111C2E] rounded-3xl border border-slate-200 dark:border-slate-800 p-8 shadow-sm">
