...
export default function MethodologySection() {
  return (
    <section className="py-16 sm:py-24 bg-brand-navy-dark text-white relative overflow-hidden border-t border-slate-800">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        
        {/* Header */}
        <div className="text-center max-w-3xl mx-auto space-y-3">
          <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-white/10 text-brand-steel-light text-xs font-semibold uppercase tracking-wider">
        ...
        <div className="mt-12 sm:mt-16 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {coreMethodologySteps.map((step, idx) => (
            <div
              key={step.step}
              className="bg-[#111C2E] rounded-2xl p-6 sm:p-7 border border-slate-800 flex flex-col justify-between"
            >
              <div>
                {/* Step Icon Badge */}
                <div
        ...
        <div className="mt-12 text-center">
          <Link
            href="/about"
            className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white/10 hover:bg-white/15 text-white text-xs sm:text-sm font-semibold border border-white/15 transition-all duration-200"
          >
            <span>Learn About Our Advisory Philosophy & Team</span>
